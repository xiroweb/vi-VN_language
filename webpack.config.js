...
//chi chay tren package thoi, 
const extensionTypesDirsPackage = [
  'package',
];
  ...
  zipDirectories.forEach((tplDirectory) => {
    ...
    extensionTypesDirs.forEach((extensionType) => {
      ...
      templates.forEach((tplFile) => {
        ...
        } else if (renamedExtElement === 'template') {
          renamedExtElement = 'tpl_' + definitions.EXTENSION_ALIAS;
        }
...
// dung de buil them cho package
function declareZipsGenerationPackage() {
  const zipDirectories = [templatesDir];
  const zipPlugins = [];
  ...
  zipDirectories.forEach((tplDirectory) => {
    // For all extension types
    extensionTypesDirsPackage.forEach((extensionType) => {
      const extZipDir = resolveExtensionTemplate(tplDirectory, extensionType);
      const templates = discoverFilesToRender(tplDirectory, extensionType);
      ...
      templates.forEach((tplFile) => {
        ...
        if (renamedExtElement === 'component') {
          ...
          renamedExtElement = 'cli';
        } else if (renamedExtElement === 'package') {
          renamedExtElement = 'pkg_' + definitions.EXTENSION_ALIAS;
        }
...
if (isPackageType()) {
  ...
  buildPlugins = buildPlugins.concat(declareZipsGenerationPackage());
